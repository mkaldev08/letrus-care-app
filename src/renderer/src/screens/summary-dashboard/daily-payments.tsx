...
import {
  useDailyPaymentsQuery,
  useDailyPaymentsExportQuery
} from '@renderer/hooks/queries/useDashboardStatistics'
import { format } from 'date-fns'
import { DailyPaymentsPDF } from '@renderer/reports/models/DailyPaymentsPDF'

export const DailyPayments: React.FC = () => {
  ...
  const { data: paymentsData, isLoading: isLoadingPayments } = useDailyPaymentsQuery(
    center?._id,
    currentSchoolYear?._id,
    currentPage
  )

  const { refetch: refetchExport } = useDailyPaymentsExportQuery(
    center?._id,
    currentSchoolYear?._id
  )

  const totalPages = useMemo(() => {
    if (!paymentsData?.total) return 1
    return Math.ceil(paymentsData.total)
  }, [paymentsData?.total])
  ...
  async function handleDownloadPDF(): Promise<void> {
    ...
    try {
      ...
      const blob = await pdf(
        <DailyPaymentsPDF data={result} center={center} user={user} />
      ).toBlob()
      const url = URL.createObjectURL(blob)
      const a = document.createElement('a')
      a.href = url
      a.download = `pagamentos-diarios-${center.documentCode}-${new Date().toLocaleDateString()}.pdf`
  ...
  const paymentsList = paymentsData?.dailyPayments ?? null
  const isLoading = isLoadingSchoolYear || isLoadingPayments

  return (
    <Layout>
      <h2 className="text-3xl text-zinc-400 mb-4">Pagamentos Diários</h2>
